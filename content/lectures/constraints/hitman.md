...
Это супер крутой метод, который придумали в 2000 году разработчики игры Hitman: Codename 47. Сейчас он используется в основном в простой симуляции ткани и веревок. Для симуляции твердых и мягких тел его уже не используют. К тому же другой метод который уже используется (XPBD) во многом похож на него.
...
Чтобы сэкономить на этом в статье вычисляют не честный корень, а его аппроксимацию
$$
\begin{equation}
\begin{split}
&d = p_2 - p_1 \\\
...
На 6 проходе уже можно останавливаться. 
...
&= (c_1 * (p_1 + c_1*\lambda * n) + c_2 * (p_2 + c_2*\lambda * n)) * n = \dots \\\
&= b * n + \lambda * (c_1^2 + c_2^2) * n^2
\end{split}
\end{equation}
$$

Таким образом мы можем найти $\lambda$:
$$
\begin{equation}
...
Но мы можем храним предыдущую позицию и для того чтобы уменьшить скорость частицы, мы можем предыдущую позиции пододвинуть к текущей. Таким образом мы уменьшим скорость частицы.

## Симуляция ткани

Этим методом можно симулировать ткань если вызывать немного итераций. А еще можно убирать констрейн, если он слишком сильно растягивается и таким образом рвать ткань. Зацените:
